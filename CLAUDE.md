...
frontend/app/
  ...
  board/page.tsx        — Public marketing post list; NO auth required
  board/[id]/page.tsx   — Public marketing post detail; NO auth required; renders MarkdownContent
...
이미지 업로드 흐름: `UploadFile` 수신 → Drive `marketing-images` 폴더에 저장 (폴더 없으면 `PARENT_DRIVE_FOLDER_ID` 하위에 자동 생성) → `anyone reader` 권한 설정 → `{"url": "https://drive.google.com/uc?export=view&id=..."}` 반환. 제한: 5MB, jpg/png/gif/webp만 허용.
...
- **`frontend/app/sitemap.ts`** — Next.js App Router 동적 sitemap. 빌드/요청 시 `GET /api/marketing/posts` 호출하여 게시된 게시물을 `/board/{id}` URL로 포함 (revalidate 1시간). API 접근 불가 시 정적 항목(`/`, `/board`)만 반환.
- **`frontend/public/robots.txt`** — 정적 파일. 크롤러 허용: `/`, `/board`. 차단: `/login`, `/dashboard`, 내부 앱 경로 전체. `Sitemap: https://www.hanwory.com/sitemap.xml` 포함.
