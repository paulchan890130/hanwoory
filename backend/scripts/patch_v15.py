"""
Patch v15 — 1 row: M1-0198 E-10 CHANGE → NOT_IN_MANUAL (empty).

DEFAULT: dry-run. `--apply` required for DB write.

Target:
  M1-0198  E-10  CHANGE
    current: 체류민원 p.332 (오매핑 — E-7 직종표 페이지)
    REPLACE → [] (NOT_IN_MANUAL)
    E-10(선원취업) CHANGE 절차는 체류민원에 별도 섹션 없음.
    p.332는 E-7-3 숙련기능인력 직종표(E-7 내부 페이지) 오매핑.
    disposition: NOT_IN_MANUAL (기등록, M1-0198)
"""
...
DRY_JSON   = ROOT / "backend" / "data" / "manuals" / "v15_patch_dryrun.json"
DRY_XLSX   = ROOT / "backend" / "data" / "manuals" / "v15_patch_dryrun.xlsx"
...
PATCHES: dict[str, dict] = {
    "M1-0198": {
        ...
        "page_evidence": (
            "E-10(선원취업) CHANGE(체류자격변경) 절차는 체류민원에 별도 섹션 없음. "
            "기존 체류민원 p.332는 E-7-3 숙련기능인력 직종표(연번 35 벌목 등) 오매핑. "
            "empty_ref_disposition.json에 NOT_IN_MANUAL로 기등록(M1-0198). "
...
def make_backup(db_bytes: bytes) -> Path:
    ...
    bk = BACKUP_DIR / f"immigration_guidelines_db_v2.v15_patch_backup_{ts}.json"
...
def write_dryrun_reports(patch_report: list, summary: dict) -> None:
    DRY_JSON.write_text(json.dumps({
        "patch_meta": {"version": "v15", "mode": "DRY_RUN",
                       "scope": "1 row: E-10 CHANGE NOT_IN_MANUAL"},
...
def main() -> int:
    ...
    if not is_apply:
        print(f"\n{'='*70}\nPATCH v15 — DRY-RUN ({len(PATCHES)} rows)\n{'='*70}")
        ...
        for x in patch_report:
            ...
            print(f"    new: []")
        print()
        print("  Apply command (NOT EXECUTED):\n    python backend/scripts/patch_v15.py --apply")
        return 0
    # APPLY
    print("\n[apply] 백업 생성...")
    bk = make_backup(raw)
