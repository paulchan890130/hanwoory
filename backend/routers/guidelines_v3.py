...
- 조회 = 전 로그인 사용자(get_current_user). 편집(CRUD) = 관리자 전용(require_admin, 비관리자 403).
"""
...
def _editable_for(user: dict) -> bool:
    """편집 UI 노출 여부 — 편집 계층 활성 + 관리자(일반 사용자는 조회 전용)."""
    return edit_store.edit_enabled() and bool(user.get("is_admin") or user.get("is_master"))
...
def edit_status(admin: dict = Depends(require_admin)):
    _require_enabled()
...
def edit_export(admin: dict = Depends(require_admin)):
...
def edit_impact(entity_type: str, entity_id: str, admin: dict = Depends(require_admin)):
    _require_editable()
    ds = get_dataset()
...
def edit_create(entity_type: str, request: Request,
                payload: dict = Body(...), admin: dict = Depends(require_admin)):
    _require_editable()
    ds = _build_merged()
    edits = edit_store.load_all_edits()
    if entity_type == "doc_requirement":
...
def edit_delete(entity_type: str, entity_id: str, request: Request,
                cascade: bool = False, admin: dict = Depends(require_admin)):
    _require_editable()
    ds = _build_merged()
    before = _current_entity(ds, entity_type, entity_id)
    if before is None:
