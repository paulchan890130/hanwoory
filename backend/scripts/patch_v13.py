...
Targets:
  E-7-S 3건 (REPLACE 사증민원 → 체류민원 p.291):
    M1-0320  E-7-S  CHANGE  → 체류민원 p.291
    M1-0321  E-7-S  EXTEND  → 체류민원 p.291
    M1-0324  E-7-S  GRANT   → 체류민원 p.291

  NOT_IN_MANUAL 1건 (REPLACE → empty []):
    M1-0290  H-1    CHANGE  → [] (NOT_IN_MANUAL, disposition 기확인)
"""
...
DRY_JSON   = ROOT / "backend" / "data" / "manuals" / "v13_patch_dryrun.json"
DRY_XLSX   = ROOT / "backend" / "data" / "manuals" / "v13_patch_dryrun.xlsx"

PATCHES: dict[str, dict] = {
    # ── E-7-S 3건 (사증민원 오매핑 → 체류민원 p.291) ──────────────
    "M1-0320": {
        "detailed_code": "E-7-S",
        "action_type":   "CHANGE",
        "title":         "네거티브방식 전문인력",
        ...
        "page_evidence": (
            "체류민원 p.291: 'V. 네거티브 방식의 전문인력 비자(E-7-S) — 2. 유형별 세부 기준 — "
            "E-7-S1(고소득자)/E-7-S2(첨단산업) (자격요건, 사증발급 및 체류관리 기준)'. "
            "E-7-S 전용 통합 체류관리 섹션 — CHANGE/EXTEND/GRANT 별도 섹션 없이 통합 기술. "
            "직접 PDF 열람 검증(2026-05-02). 기존 사증민원 p.247-249 오매핑 제거."
        ),
        "replace_all": True,
        "add_entries": [
            {
                ...
                "match_text": "E-7-S 네거티브방식 CHANGE 체류관리 (체류민원 p.291) — 수동 검증",
            },
        ],
    },
    "M1-0321": {
        ...
        "page_evidence": (
            "체류민원 p.291: 'V. 네거티브 방식의 전문인력 비자(E-7-S) — 2. 유형별 세부 기준 — "
            "(자격요건, 사증발급 및 체류관리 기준)'. "
            "E-7-S 통합 섹션 — '체류관리 기준'이 체류기간연장 포함. "
            "직접 PDF 열람 검증(2026-05-02). 기존 사증민원 p.247 오매핑 제거."
        ),
        "replace_all": True,
        "add_entries": [
            {
                ...
                "page_to":    291,
                "match_type": "manual_override",
                "match_text": "E-7-S 네거티브방식 EXTEND 체류관리 (체류민원 p.291) — 수동 검증",
            },
        ],
    },
    "M1-0324": {
        ...
        "preferred_manual": "체류민원",
        "page_evidence": (
            "체류민원 p.291-292: 'V. 네거티브 방식의 전문인력 비자(E-7-S) — "
            "2. 유형별 세부 기준 — (자격요건, 사증발급 및 체류관리 기준)' + "
            "p.292 '3. 거주(F-2) 및 영주(F-5) 자격 취득에 대한 특례'. "
            "E-7-S 통합 섹션 — '체류관리 기준'이 체류자격부여(GRANT) 포함. "
            "직접 PDF 열람 검증(2026-05-02). 기존 사증민원 p.247-249 오매핑 제거."
        ),
        "replace_all": True,
        "add_entries": [
            {
                ...
                "page_to":    291,
                "match_type": "manual_override",
                "match_text": "E-7-S 네거티브방식 GRANT 체류관리 (체류민원 p.291) — 수동 검증",
            },
        ],
    },
    # ── M1-0290 H-1 CHANGE → NOT_IN_MANUAL (empty) ────────────────
    "M1-0290": {
        "detailed_code": "H-1",
        "action_type":   "CHANGE",
        "title":         "관광취업",
        "preferred_manual": "체류민원",
        "page_evidence": (
            "H-1 관광취업 CHANGE 절차는 체류민원에 별도 섹션 없음. "
            "empty_ref_disposition.json에 NOT_IN_MANUAL로 기등록. "
            "기존 사증민원 p.336 매핑은 관광취업 자격 개요 페이지 오매핑. "
            "manual_ref → [] (빈 배열)로 정리."
        ),
        "replace_all": True,
        "add_entries": [],  # empty = NOT_IN_MANUAL
    },
}
...
def make_backup(db_bytes: bytes) -> Path:
    ...
    bk = BACKUP_DIR / f"immigration_guidelines_db_v2.v13_patch_backup_{ts}.json"
...
def build_patched_db(db: dict) -> tuple:
    ...
    for rid, spec in PATCHES.items():
        ...
        if spec.get("replace_all"):
            new_ref = list(spec["add_entries"])  # empty list for NOT_IN_MANUAL
        else:
...
def write_dryrun_reports(patch_report: list, summary: dict) -> None:
    DRY_JSON.write_text(json.dumps({
        "patch_meta": {"version": "v13", "mode": "DRY_RUN",
                       "scope": "4 rows: E-7-S(3) CHANGE/EXTEND/GRANT + H-1 CHANGE NOT_IN_MANUAL"},
...
def main() -> int:
    ...
    if not is_apply:
        print(f"\n{'='*70}\nPATCH v13 — DRY-RUN ({len(PATCHES)} rows)\n{'='*70}")
        ...
        print()
        for x in patch_report:
            mode = "REPLACE→empty(NOT_IN_MANUAL)" if x["replace_all"] and not x["added_entries"] else ("REPLACE" if x["replace_all"] else "ADD")
            ...
            for e in x["old_manual_ref"]:
                print(f"      - {e.get('manual')} p.{e.get('page_from')}-{e.get('page_to')}")
            for e in x["new_manual_ref"]:
                print(f"      + {e.get('manual')} p.{e.get('page_from')}-{e.get('page_to')}  [{e.get('match_text','')[:70]}]")
        print()
        print("  Apply command (NOT EXECUTED):\n    python backend/scripts/patch_v13.py --apply")
        return 0
    # APPLY
    print("\n[apply] 백업 생성...")
    bk = make_backup(raw)
