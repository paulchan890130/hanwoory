"""
Patch v11 — 3 rows (2 pre-confirmed HIGH + 1 Step 1 HIGH).

DEFAULT: dry-run. `--apply` required for DB write.

Targets:
  Pre-confirmed HIGH:
    M1-0141  E-1     VISA_CONFIRM  → ADD 사증민원 p.141 (교수 사증발급인정서)
                                      기존 체류민원 p.163-166 유지
    M1-0015  E-10-2  WORKPLACE     → REPLACE → 체류민원 p.333-334 (근무처변경)
                                      기존 사증민원 p.294-296 오매핑 제거

  Step 1 HIGH (사증민원 탐색 결과):
    M1-0199  E-10    VISA_CONFIRM  → ADD 사증민원 p.294-296 (선원취업 사증발급인정서)
                                      기존 체류민원 p.332 오매핑 유지 (replace_all=False)
"""
...
DRY_JSON   = ROOT / "backend" / "data" / "manuals" / "v11_patch_dryrun.json"
DRY_XLSX   = ROOT / "backend" / "data" / "manuals" / "v11_patch_dryrun.xlsx"
...
PATCHES: dict[str, dict] = {
    ...
    "M1-0141": {
        ...
        "page_evidence": (
            "사증민원 p.141: '사증발급인정서 발급대상 — 1. 전문대학 이상 교육기관에서 91일 이상 "
            "교육 또는 연구지도활동을 하는 대학의 장(총장학장), 교수부교수조교수, 겸임교원명예교수초빙교원, "
            "교환교수 등' + 교수 자격기준표(연구실적연수+교육경력연수 합산) + 첨부서류 "
            "(①사증발급인정신청서 ②경력증명서·학위증 ③고용계약서 ④초청학교 설립관련 서류). "
            "직접 PDF 열람 검증. 기존 체류민원 p.163-166 entry 유지."
        ),
        "replace_all": False,
        "add_entries": [
            {
                ...
                "match_text": "E-1 교수 VISA_CONFIRM 사증발급인정서 (사증민원 p.141) — 수동 검증",
            },
        ],
    },
    "M1-0015": {
        "detailed_code": "E-10-2",
        "action_type":   "WORKPLACE",
        "title":         "어선원 근무처변경",
        "preferred_manual": "체류민원",
        "page_evidence": (
            "체류민원 p.333-334: E-10 선원취업 챕터 내 '근무처의 변경·추가' 섹션 — "
            "'선원취업자가 사업장의 휴·폐업 등으로 정상적인 근로관계를 지속하기 곤란한 때에는 "
            "지방해양수산청장의 추천을 받아 근무처 변경 신청 가능' + 제출서류(①신청서 "
            "②한국해운조합 또는 수산업협동조합중앙회 발급 외국인 고용추천서 ③고용계약서 "
            "④사업자등록증 ⑤산재서류·진단서 ⑥신원보증서). "
            "기존 사증민원 p.294-296은 VISA_CONFIRM 섹션으로 WORKPLACE 오매핑. 전체 교체."
        ),
        "replace_all": True,
        "add_entries": [
            {
                ...
                "page_to":    334,
                "match_type": "manual_override",
                "match_text": "E-10-2 선원취업 WORKPLACE 근무처변경 (체류민원 p.333-334) — 수동 검증",
            },
        ],
    },
    # ── Step 1 HIGH ───────────────────────────────────────────────
    "M1-0199": {
        ...
        "page_evidence": (
            ...
            "p.294 활동범위·해당자(E-10-1 내항선원/E-10-2 어선원/E-10-3 순항여객선원), "
            "p.295 '선원취업(E-10) 사증은 출입국·외국인청(사무소·출장소)장이 발급한 "
            "사증발급인정서에 의해서만 발급' + 도입규모 결정과정 + 접수처리 절차, "
            "p.296 사증발급인정서 발급대상 첨부서류(공통 4종 + E-10-2 어선원 전용 4종). "
            "직접 PDF 열람 검증. 기존 체류민원 p.332 entry(오매핑)는 ADD이므로 유지."
        ),
        "replace_all": False,
        "add_entries": [
            {
...
def make_backup(db_bytes: bytes) -> Path:
    ...
    bk = BACKUP_DIR / f"immigration_guidelines_db_v2.v11_patch_backup_{ts}.json"
...
def write_dryrun_reports(patch_report: list, summary: dict) -> None:
    DRY_JSON.write_text(json.dumps({
        "patch_meta": {"version": "v11", "mode": "DRY_RUN",
                       "scope": "3 rows: E-1/E-10-2/E-10"},
...
def main() -> int:
    ...
    if not is_apply:
        print(f"\n{'='*70}\nPATCH v11 — DRY-RUN ({len(PATCHES)} rows)\n{'='*70}")
        ...
        print()
        print("  Apply command (NOT EXECUTED):\n    python backend/scripts/patch_v11.py --apply")
        return 0
    # APPLY
    print("\n[apply] 백업 생성...")
    bk = make_backup(raw)
