...
종료코드 (no-op skip 은 정상 성공으로 분류 — Cron 오탐 방지):
    0 — staging 이 staged / no_change / **skipped**(already ran today · locked ·
        pg_disabled 는 정상 no-op). PDF 가 with_pdf=False 등으로 skipped 인 경우도 0.
    1 — staging status == error(실제 예외 · 다운로드/파싱 실패 · DB 오류) 이거나
        알 수 없는 status 일 때, 또는 PDF 단계 status == error 일 때(자동화 실패로 간주).
...
def main(argv: list[str] | None = None) -> int:
    ...
    pdf_status = pdf.get("status") if isinstance(pdf, dict) else None

    # no-op skip(already ran today / locked / pg_disabled)과 staged/no_change 는 정상 → 0.
    # 실패(1)는 staging 이 명시적 error 이거나 알 수 없는 status, 또는 PDF 단계가 error 일 때만.
    _SUCCESS_STAGING = ("staged", "no_change", "skipped")
    failed = (staging_status not in _SUCCESS_STAGING) or (pdf_status == "error")
