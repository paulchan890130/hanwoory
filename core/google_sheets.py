...
if _in_fastapi:
    ...
    _fake_st = _types.SimpleNamespace(
        ...
        error=lambda *a, **k: print("[st.error]", *a),
        warning=lambda *a, **k: print("[st.warning]", *a),
    )
    _fake_st.cache_data.clear = lambda: None
    st = _fake_st
# ─────────────────────────────────────────────────────────────────────────────
...
def get_gspread_client():
    ...
    import os as _os
    if RUN_ENV == "server" or _os.path.isfile(KEY_PATH):
        # 서비스 계정 키 파일이 있으면 항상 서비스 계정 사용 (로컬 포함)
        creds = Credentials.from_service_account_file(KEY_PATH, scopes=scopes)
    else:
        # 키 파일 없을 때만 OAuth 시도 (구 로컬 방식)
...
def _get_spreadsheet_cached(sheet_key: str):
    """
    Spreadsheet 메타데이터 객체를 캐싱.
    open_by_key() 는 매번 API 호출(메타데이터 fetch)을 유발하므로
    600 초 동안 재사용해 Read quota 절약.
    """
    client = get_gspread_client()
...
def get_worksheet(client, sheet_name: str):
    ...
    sh = _get_spreadsheet_cached(sheet_key)
...
def upsert_rows_by_id(sheet_name: str,
    ...
    try:
        ...
        try:
            ...
            # 시트(탭)가 없으면 자동 생성
            sh = _get_spreadsheet_cached(SHEET_KEY)
            ws = sh.add_worksheet(title=sheet_name, rows=1000, cols=len(header_list) + 2)
...
def delete_rows_by_ids(sheet_name: str, rids: list, id_field: str = "id") -> bool:
    """
    여러 id 를 한 번의 get_all_values() 로 위치 파악 후 일괄 삭제.
    delete_row_by_id() 를 N번 호출하는 대신 API 호출 횟수를 최소화.
    """
    ...
    try:
        ...
        # ✅ 역순 삭제: 위 행을 먼저 삭제하면 아래 행번호가 밀리므로 역순으로
        for row_no in reversed(rows_to_delete):
            ws.delete_rows(row_no)

        return True
    except Exception as e:
        st.error(f"❌ delete_rows_by_ids 오류 ({sheet_name}): {e}")
        return False
...
def append_rows_to_sheet(sheet_name: str, records: list[dict], header_list: list[str]) -> bool:
    ...
    value_input_option="USER_ENTERED" → 한글/특수문자 포함 모든 값 안전 저장.
    """
    try:
        client    = get_gspread_client()
        worksheet = get_worksheet(client, sheet_name)
        rows = [[str(record.get(h, "")) for h in header_list] for record in records]
...
def read_data_from_sheet(sheet_name: str, default_if_empty=None):
    ...
    try:
        ...
        # 헤더 중복 오류 시 get_all_values() 로 직접 파싱 (첫 번째 컬럼명만 사용)
        if "duplicate" in str(e).lower():
            try:
                ...
                header = values[0]
                seen_cols: set = set()
                col_map: list = []          # (시트_인덱스, 컬럼명) - 중복 제거 후
                ...
                records = []
                for row in values[1:]:
                    rec = {col: (row[i] if i < len(row) else "") for i, col in col_map}
                    records.append(rec)
                return records if records else default_if_empty
            except Exception as e2:
                print(f"[시트 읽기 실패] {sheet_name}: {e2}")
                return default_if_empty
