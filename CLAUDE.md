...
backend/
  ...
  data/
    ...
    immigration_guidelines_db_v2.json — 출입국 실무지침 DB (~657KB, 363 업무항목)
  scripts/
...
frontend/app/
  ...
  (main)/guidelines/    — 출입국 실무지침 DB viewer (code search, action-type tabs, TB banner, quickdoc deeplink)
...
### 데이터 소스
- `backend/data/immigration_guidelines_db_v2.json` (~651KB)
- 원본 엑셀(`정리.xlsx`) → `엑셀_json_변환.py` 스크립트로 재생성 가능
- 포함 시트: MASTER_ROWS(363건), RULES(38건), EXCEPTIONS(32건), DOC_DICTIONARY(36건), SEARCH_KEYS, LEGACY_UI_MAP
...
| `GET /tree/entry-points` | 상담 진입점 13개 + 각 업무 건수 |
